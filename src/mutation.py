...
def mutate_circuit(qc, mutation_idx, quantum_gates) -> QuantumCircuit:
    ...
    for current_idx, (instr, qargs, cargs) in enumerate(qc.data):
        ...
        if current_idx == mutation_idx:
            ...
            # Get random replacement gate. Normalize params so that None and
            # [] compare equal: otherwise drawing the SAME non-parameterized
            # gate (old params [], new params None) exits the loop and yields
            # a no-op "mutation".
            new_gate_name = copy.deepcopy(old_gate_name)
            new_gate_params = copy.deepcopy(old_gate_params)

            while (new_gate_name == old_gate_name
                   and list(new_gate_params or []) == list(old_gate_params or [])):
